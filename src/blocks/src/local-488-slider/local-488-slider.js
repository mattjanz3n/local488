...
const { Button, PanelBody, IconButton, TextareaControl, TextControl, SelectControl } = wp.components;
...
registerBlockType("cgb/block-local-488-slider", {
	...
	edit: (props) => {
		...
		if (props.attributes.images.length) {
						...
						<div className="wrapper-image-repeater">
							<div className="image">
								...
								<MediaUpload
									...
									type="image"
									value={props.attributes.images[index].imageID}
									render={({ open }) => getImageButton(open)}
								/>
								<img
									className="content-image"
									src={props.attributes.images[index].imageUrl}
									draggable="false"
								/>
							</div>
						</div>
							<TextareaControl
								onChange={ ( caption ) => {
									...
									value={ props.attributes.images[index].imageCaption }
							...
							<TextControl
								...
								value={ props.attributes.images[index].imagePaddings }
								type="number"
								placeholder="0px"
							/>
					</Fragment>
			...
			imageDisplay = props.attributes.images.map((image, index) => {
				return (
					<figure className="image-slide__wrapp" style={{paddingTop: image.imagePaddings}}>
						<img
			...
			imagePreview = (
				<div className="image__section">
					<div className={`wrapper wrapper--width-${props.attributes.sectionWidth}`}>
			...
			<InspectorControls key="1">
				<PanelBody title={__("Section Width")}>
					<SelectControl
						...
						onChange={ ( sectionWidth ) => props.setAttributes({sectionWidth}) }
					/>
				</PanelBody>
	...
	save: (props) => {
		const imageFields = props.attributes.images.map((image, index) => {
			...
			return (
				<figure className="image-slide__wrapp" style={{paddingTop: image.imagePaddings === '' ? '0px' : `${image.imagePaddings}px`, paddingBottom: image.imagePaddings === '' ? '0px' : `${image.imagePaddings}px`}}>
					<img
						className="image-slide"
						data-image-index={index}
						src={image.imageUrl}
