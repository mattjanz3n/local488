...
export default function categoriesNewsAndEventsPostsFilter() {
	...
	function categoriesNewsAndEventsPostsSort(categoryCurrent, paged) {
		...
		let postWrapNewsAndEvents = $('.news-and-events-content-section__wrapper');
	...
	$('.news-and-events-content-section__button').on('click', function (e) {
		e.preventDefault();
		$(this).toggleClass('active');
		const categoryCurrent = $('.news-and-events-content-section__button.active').get().map(n => n.dataset['slug']);
		let paged = 1;
	...
	$('.loc-single-post__categories-link').on('click', function (e) {
		e.preventDefault();
		let categoryCurrent = $('.archive-category-button.active').get().map(n => n.dataset['slug']);
		let paged = 1;
		categoriesNewsAndEventsPostsSort(categoryCurrent, paged);
	});
	...
	$(document).on('click', 'a.page', function (e) {
		...
		if (page == 'prev') {
			$('a.pagination.active').parent().prev().children('a.pagination').trigger('click');

		} else {
			$('a.pagination.active').parent().next().children('a.pagination').trigger("click");
		}
	});
}
