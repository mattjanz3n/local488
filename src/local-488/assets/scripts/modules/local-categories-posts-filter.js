...
export default function categoriesPostsFilter() {
	...
	$('.community-involvement-button').on('click', function (e) {
		...
		const categoryCurrent = $('.archive-category-button.active').get().map(n => n.dataset['slug']);
		let paged = 1;
		categoriesPostsSort(categoryCurrent, paged);
	});

	$(document).on('click', '.community-involvment-article__categories-link', function (e) {
		e.preventDefault();
