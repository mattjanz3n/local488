<?php
/**
 * Various functions required for local_488-bootstrap to work properly
 *
 * @package Local_488
 */
...
/**
 * local_488_navwalker class
 * Menu navigation walker
 * Class Name: local_488_navwalker
 * GitHub URI: https://github.com/twittem/wp-local_488-navwalker
 * Description: A custom WordPress nav walker class to implement the local_488 3 navigation style in a custom theme using the WordPress built in menu manager.
 */
class Local_488_Navwalker extends Walker_Nav_Menu {
	/**
	 * Walker::start_lvl()
	 *
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( strcasecmp( $item->attr_title, 'divider' ) === 0 && $depth === 1 ) {
			...
			if ( $args->has_children && $depth === 0 ) {
				$class_names .= ' dropdown dropdown-firs-level';
			}
			if ( $args->has_children && $depth === 1 ) {
				$class_names .= ' dropdown dropdown-second-level';
			}
			if ( $args->has_children && $depth === 2 ) {
				$class_names .= ' dropdown dropdown-third-level';
			}
...
/**
 * Pagination
 * Custom pagination with local_488 .pagination class
 * source: http://www.ordinarycoder.com/paginate_links-class-ul-li-local_488/
 *
 * @param boolean $echo echo
 * @return string
 */
function local_488_pagination( $echo = true ) {
	global $wp_query;
...
/**
 * 4 - Comments tree
 * local_488 Comments Tree
 */
if ( ! class_exists( 'Local_488_Comments' ) ) :
	/**
	 * Undocumented class
	 */
	class Local_488_Comments extends Walker_Comment {
		/**
		 * tree_type
		 *
		...
		public function __construct() { ?>
			<h3><?php comments_number( __( 'No Responses to', 'local_488' ), __( 'One Response to', 'local_488' ), __( '% Responses to', 'local_488' ) ); ?> &#8220;<?php the_title(); ?>&#8221;</h3>
